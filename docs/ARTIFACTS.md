# Artifacts

## Trait x Rarity

| Trait | Common | Uncommon | Rare | Epic | Legendary | Total |
|---|---|---|---|---|---|---|
| Outlaw | 1 | 1 | 3 | 0 | 2 | 7 |
| Sheriff | 3 | 1 | 4 | 0 | 0 | 8 |
| Antivenom | 0 | 1 | 1 | 0 | 2 | 4 |
| Rattlesnake | 1 | 2 | 1 | 0 | 1 | 5 |
| Prospector | 1 | 3 | 3 | 0 | 1 | 8 |
| Sapper | 1 | 1 | 1 | 0 | 2 | 5 |
| Mustang | 3 | 0 | 2 | 0 | 1 | 6 |
| Gunslinger | 2 | 3 | 1 | 0 | 2 | 8 |
| Saloon Keeper | 1 | 2 | 3 | 0 | 0 | 6 |
| Sniper | 0 | 1 | 3 | 0 | 1 | 5 |
| Dead Man Walking | 0 | 2 | 3 | 0 | 3 | 8 |
| Preacher | 1 | 3 | 1 | 0 | 2 | 7 |
| Tracker | 3 | 1 | 0 | 0 | 1 | 5 |
| Undertaker | 1 | 3 | 0 | 0 | 2 | 6 |
| Desperado | 1 | 3 | 1 | 0 | 1 | 6 |

Overall: 19C, 27U, 27R, 0E, 21L = 94 trait slots across 62 artifacts

## Trait Intersection Matrix

Diagonal = single-trait artifacts. Off-diagonal = artifacts with both traits.

| | OUT | SHR | AV | RAT | PRO | SAP | MUS | GUN | SAL | SNI | DMW | PRE | TRK | UND | DES |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| **OUT** | 0 | 1 | . | . | . | . | 1 | 2 | . | 2 | . | . | . | 1 | . |
| **SHR** | | 4 | . | . | . | . | . | 1 | . | . | 1 | . | . | . | 1 |
| **AV** | | | 2 | . | . | 1 | . | . | . | . | 1 | . | 1 | . | . |
| **RAT** | | | | 3 | . | . | . | 1 | . | . | . | . | . | . | 1 |
| **PRO** | | | | | 4 | 1 | . | . | . | . | . | 1 | 1 | . | 1 |
| **SAP** | | | | | | 2 | . | . | . | . | . | . | 1 | 1 | . |
| **MUS** | | | | | | | 3 | . | . | . | . | 1 | 1 | . | . |
| **GUN** | | | | | | | | 1 | 1 | . | 1 | 1 | . | . | . |
| **SAL** | | | | | | | | | 3 | . | 1 | 1 | . | . | . |
| **SNI** | | | | | | | | | | 3 | . | . | . | . | . |
| **DMW** | | | | | | | | | | | 1 | 1 | . | 1 | 1 |
| **PRE** | | | | | | | | | | | | 1 | . | 1 | . |
| **TRK** | | | | | | | | | | | | | 1 | 1 | . |
| **UND** | | | | | | | | | | | | | | 1 | . |
| **DES** | | | | | | | | | | | | | | | 2 |

## Trait Capacity

| Trait | Max Breakpoint | Artifacts | Delta |
|---|---|---|---|
| Outlaw | 5 | 7 | +2 |
| Sheriff | 6 | 8 | +2 |
| Antivenom | 3 | 4 | +1 |
| Rattlesnake | 4 | 5 | +1 |
| Prospector | 6 | 8 | +2 |
| Sapper | 4 | 5 | +1 |
| Mustang | 4 | 6 | +2 |
| Gunslinger | 6 | 8 | +2 |
| Saloon Keeper | 5 | 6 | +1 |
| Sniper | 5 | 5 | 0 |
| Dead Man Walking | 7 | 8 | +1 |
| Preacher | 6 | 7 | +1 |
| Tracker | 3 | 5 | +2 |
| Undertaker | 6 | 6 | 0 |
| Desperado | 5 | 6 | +1 |

## Exclusive

### Bamboo Canteen
- Rust exclusive starting artifact
- Tags: Saloon Keeper
- Rarity: Rare
- Effect: After completing combat, restore 6 HP.
- Flavor: "Bamboo doesn't rust."

### Rust's Cylinder
- Rust exclusive artifact
- Tags: Gunslinger, Outlaw
- Rarity: Legendary
- Effect: Increase Deadeye shots to 6. The last shot can now be used on an enemy, dealing 7 damage plus 1 damage per stack of Bounty.
- Flavor: "Six in the chamber. No reloads."

### Rigged Deck
- Reno exclusive starting artifact
- Tags: Prospector
- Rarity: Rare
- Effect: Chip hits have a 50% chance to hit another enemy. Chip misses generate 2 gold.
- Flavor: "Bad luck is still luck."

### Reno's Coin
- Reno exclusive artifact
- Tags: Desperado
- Rarity: Legendary
- Effect: Chip damage is doubled and hit chance is increased to 75%. On miss, lose 1 HP.
- Flavor: "Double or nothing. Emphasis on nothing."

### Used Cigarette
- Rudy exclusive starting artifact
- Tags: Sheriff
- Rarity: Rare
- Effect: Gain 10 block after taking damage for the first time each combat.
- Flavor: "Still got some left in him."

### Hard Exterior
- Rudy exclusive artifact
- Tags: Sheriff
- Rarity: Legendary
- Effect: Gain 10 max HP and 20 Hardened.
- Flavor: "I can do this all day."

## Outlaw

### Outlaw's Spurs
- Tags: Outlaw, Mustang
- Rarity: Common
- Effect: At the start of combat, gain 1 Ready.
- Flavor: "Kick the door in. Ask questions later."

### Twin Revolvers
- Tags: Outlaw, Gunslinger
- Rarity: Uncommon
- Effect: Bullets deal 50% more damage, but has a 10% chance to miss.
- Flavor: "Two guns. Twice the fun."

### Stolen Badge
- Tags: Sheriff, Outlaw
- Rarity: Rare
- Effect: At the start of combat, gain 20 block. Merchants cost 10% more.
- Flavor: "Wanted fugitive. Merchants charge extra."

## Sheriff

### Rusty Deputy Badge
- Tags: Sheriff
- Rarity: Common
- Effect: At the start of the second turn each combat, gain 13 block.
- Flavor: "Tarnished but still standing."

### Bounty Board
- Tags: Sheriff
- Rarity: Common
- Effect: Whenever an enemy is summoned, gain 1 Rageful.
- Flavor: "Every menace has a price."

### Jail Cell Keys
- Tags: Sheriff
- Rarity: Uncommon
- Effect: Whenever you clear a locked tile, gain 4 block.
- Flavor: "Justice unlocked."

### Reinforced Duster
- Tags: Sheriff
- Rarity: Rare
- Effect: Start each fight with 3 Sturdy.
- Flavor: "Lined with steel plates."

### Iron Will
- Tags: Sheriff, Dead Man Walking
- Rarity: Rare
- Effect: At the end of your turn, when you are at or below 20% HP, gain 4 block.
- Flavor: "Bent but never broken."

## Antivenom

### Snakeskin Boots
- Tags: Antivenom
- Rarity: Uncommon
- Effect: First poison tile per turn is auto-cleansed.
- Flavor: "Walk through the venom."
- NOTE: If the enemy applies 3 poison, only the first one is prevented, i.e. 2 get applied to your tiles

### Bone Charm
- Tags: Antivenom
- Rarity: Rare
- Effect: Poison damage you take is halved.
- Flavor: "Carved from something that refused to stay dead."
- NOTE: New artifact

### Shed Skin
- Tags: Antivenom, Dead Man Walking
- Rarity: Legendary
- Effect: When taking lethal damage, prevent it and heal 50% HP, then destroy this artifact.
- Flavor: "Left the old self behind."
- NOTE: Dead man walking b7 should trigger before this if it is active.

## Rattlesnake

### Sidewinder Belt
- Tags: Rattlesnake
- Rarity: Common
- Effect: At the start of combat, apply 2 Poison to ALL enemies.
- Flavor: "They were poisoned before they knew you arrived."
- NOTE: this does not stack with Rattlesnake b4

### Envenomed Ammo
- Tags: Rattlesnake, Gunslinger
- Rarity: Uncommon
- Effect: Bullet-type tile matches apply 1 Poison to target.
- Flavor: "Dipped in something nasty. Every shot bites."

### Cactus Spine Vest
- Tags: Rattlesnake
- Rarity: Rare
- Effect: Whenever an enemy damages you, apply 3 Poison stacks to them.
- Flavor: "Touch me and you'll regret it."
- NOTE: only when damaging HP, doesn't apply if blocked

### Rattlesnake Fang Necklace
- Tags: Rattlesnake
- Rarity: Legendary
- Effect: Whenever you would apply Poison, apply 1 more.
- Flavor: "Trophy from the first one that bit you."
- NOTE: this also makes matching poison tiles apply 2 Poison to yourself instead of 1

## Prospector

### Gold Tooth
- Tags: Prospector
- Rarity: Rare
- Effect: Upon pickup, gain 333 gold.
- Flavor: "Smiles and profits."

## Sapper

### Blasting Pan
- Tags: Sapper, Prospector
- Rarity: Common
- Effect: Defusing bombs grants 8 gold.
- Flavor: "One man's bomb is another man's payday."

### Black Powder Cache
- Tags: Sapper
- Rarity: Rare
- Effect: At the start of combat, make 3 tiles explosive.
- Flavor: "Surplus inventory."

### Tinker's Wrench
- Tags: Sapper
- Rarity: Legendary
- Effect: Explosive tiles also spawn from the first non-cascade 3-match each turn.
- Flavor: "Every match is an explosion waiting to happen."

## Mustang

### Saddlebag
- Tags: Mustang
- Rarity: Common
- Effect: Upon pickup, gain 2 consumable slots.
- Flavor: "Room for two more."

### Trailblazer's Compass
- Tags: Mustang, Tracker
- Rarity: Common
- Effect: Unused swaps at turn end gives you 6 block on your next turn.
- Flavor: "Every path not taken is a bullet saved."

### Dust Devil Boots
- Tags: Mustang
- Rarity: Rare
- Effect: After using all swaps in a turn, shuffle the bottom 2 rows.
- Flavor: "Kick up dust."

### Dan's Feather
- Tags: Mustang
- Rarity: Legendary
- Effect: Increase swaps per turn by 1 for the rest of the run.
- Flavor: "I am speed."

## Gunslinger

### Lucky Bullet
- Tags: Gunslinger
- Rarity: Common
- Effect: At the start of combat, gain 1 Lucky.
- Flavor: "One lucky round. Make it count."

### Dead Man's Hand
- Tags: Gunslinger, Dead Man Walking
- Rarity: Uncommon
- Effect: Lucky triggers apply 1 Vulnerable.
- Flavor: "Aces and eights. Bad luck for them."

### Silver Bullet
- Tags: Gunslinger, Sheriff
- Rarity: Rare
- Effect: Whenever a boss gains block, gain 1 Ready.
- Flavor: "Saved for the big one."

## Saloon Keeper

### Barkeep's Shotgun
- Tags: Saloon Keeper, Gunslinger
- Rarity: Common
- Effect: Using a consumable deals 5 damage to the targeted enemy.
- Flavor: "And stay out."
- NOTE: New artifact

### Top-Shelf Reserve
- Tags: Saloon Keeper
- Rarity: Uncommon
- Effect: Using a consumable grants 6 block.
- Flavor: "Dutch courage in a glass."
- NOTE: New artifact

### Last Call Bell
- Tags: Saloon Keeper
- Rarity: Rare
- Effect: Consumables trigger twice.
- Flavor: "Ring the bell. One for the road."

## Sniper

### Gillie Suit
- Tags: Sniper
- Rarity: Rare
- Effect: First 5+ match each combat grants 1 Grace.
- Flavor: "They never saw you coming."
- NOTE: New artifact

### Kill Confirmed
- Tags: Sniper, Outlaw
- Rarity: Rare
- Effect: Whenever you kill an enemy with .50 Cal or a 5+ match, gain 1 swap.
- Flavor: "Target down. Moving to next."
- NOTE: New artifact

## Dead Man Walking

### Death's Pocket Watch
- Tags: Dead Man Walking
- Rarity: Rare
- Effect: Once per combat, when HP drops below 20%, gain 20 block.
- Flavor: "Time stops for the dying."
- NOTE: New artifact

## Preacher

### Preacher's Bible
- Tags: Preacher
- Rarity: Common
- Effect: Take 1 less damage from enemy attacks.
- Flavor: "Scripture is armor."

### Offering Plate
- Tags: Preacher, Prospector
- Rarity: Uncommon
- Effect: Healing grants gold equal to HP healed.
- Flavor: "Generosity is rewarded."
- NOTE: New artifact

## Tracker

### Trapper's Snare
- Tags: Tracker
- Rarity: Common
- Effect: Revealing a Buried tile applies 1 Vulnerable to a random enemy.
- Flavor: "Step on the trap. Suffer the consequence."
- NOTE: New artifact

## Undertaker

### Gravedigger's Shovel
- Tags: Undertaker, Tracker
- Rarity: Common
- Effect: Buried tiles that you uncover deal 3 damage to a random enemy.
- Flavor: "Every grave dug is a job done."

### Corpse Explosion
- Tags: Undertaker
- Rarity: Uncommon
- Effect: When a summoned enemy dies, deal damage equal to that enemy's max HP to ALL enemies.
- Flavor: "Burn the remains. Scorch the rest."
- NOTE: New artifact

### Reaper's Scythe
- Tags: Undertaker, Outlaw
- Rarity: Legendary
- Effect: Killing an enemy applies Shadow to 5 tiles on your board.
- Flavor: "Death reclaims what's owed."
- NOTE: New artifact

## Desperado

### Sheriff's Domino
- Tags: Desperado, Sheriff
- Rarity: Common
- Effect: When blocking all enemy attacks in a turn, gain 7 block at the start of your next turn.
- Flavor: "Try, uh... seven?"
- NOTE: only happens if at least 1 enemy actually deals damage, but the player blocks all of it 

### Loaded Dice
- Tags: Desperado
- Rarity: Uncommon
- Effect: Cascade matches trigger 1 level higher.
- Flavor: "The house always wins."
- NOTE: if the player's tile is Lv 2, cascade trigger of this tile will be lv 3

### Holy Water
- Tags: Preacher, Mustang
- Rarity: Rare
- Effect: Unused swaps at turn end heal you for 3 HP each.
- Flavor: "The river heals those who stop running."

### Resurrecting Nails
- Tags: Dead Man Walking, Preacher
- Rarity: Legendary
- Effect: On the 3rd turn during boss combat, restore 30% HP.
- Flavor: "On the third day, he rose."

### Dead Man's Bones
- Tags: Dead Man Walking, Desperado
- Rarity: Uncommon
- Effect: When at or below 20% HP, gain 4 Ace at the start of each turn.
- Flavor: "Nothing left to lose. Everything left to prove."

### Absolution Rounds
- Tags: Preacher, Gunslinger
- Rarity: Legendary
- Effect: Healing deals damage to the target enemy equal to the HP healed.
- Flavor: "Forgiveness comes in lead."

### Last Breath Tonic
- Tags: Dead Man Walking, Saloon Keeper
- Rarity: Rare
- Effect: Once per combat, when HP drops below 20%, gain a random consumable.
- Flavor: "One more for the road."

### Temperance Flask
- Tags: Preacher, Saloon Keeper
- Rarity: Uncommon
- Effect: Healing consumables restore 50% more HP.
- Flavor: "Discipline makes the medicine stronger."

### Scope Lens
- Tags: Sniper
- Rarity: Legendary
- Effect: 5+ matches generate double resources.
- Flavor: "See farther. Hit harder."

### Detonator
- Tags: Sapper, Undertaker
- Rarity: Uncommon
- Effect: When an enemy dies, make 1 random tile explosive.
- Flavor: "Rig the remains. Waste nothing."

### Snake Eye
- Tags: Desperado, Rattlesnake
- Rarity: Uncommon
- Effect: Cascade matches have a 50% chance to apply 1 Poison to all enemies.
- Flavor: "Bad roll. Worse bite."

### Golden Shovel
- Tags: Prospector, Tracker
- Rarity: Uncommon
- Effect: Whenever you reveal a Buried tile, gain 7 gold.
- Flavor: "X marks the payday."

### High Vis Jacket
- Tags: Antivenom, Sapper, Tracker
- Rarity: Legendary
- Effect: At the start of combat, gain 1 Protected.
- Flavor: "They can see you coming. Doesn't matter."

### Golden Scarab
- Tags: Prospector
- Rarity: Legendary
- Effect: All gold gain is increased by 30%.
- Flavor: "Everything it touches turns to gold."

### Burial Rites
- Tags: Preacher, Undertaker
- Rarity: Uncommon
- Effect: When a summoned enemy dies, heal 5 HP.
- Flavor: "Rest now. Your fight is done."

### Sniper's Eye
- Tags: Sniper, Outlaw
- Rarity: Rare
- Effect: 5-match attacks deal damage to ALL enemies.
- Flavor: "One shot. Every target."

### Heliograph Shard
- Tags: Sniper
- Rarity: Uncommon
- Effect: Once per combat, on 5-match, apply 1 Blinded to a random enemy.
- Flavor: "A flash of light. Then nothing."

### Death's Glare
- Tags: Undertaker, Dead Man Walking
- Rarity: Legendary
- Effect: On combat start, gain 3 Dead Man Walking and 3 Ready, then apply 3 Vulnerable and 3 Terrified to ALL enemies.
- Flavor: "They saw what was coming."

### Strong Liver
- Tags: Desperado, Prospector
- Rarity: Rare
- Effect: At the start of each turn, lose 1 gold and gain 1 Rageful, 1 Sturdy, 1 Ace, 1 Lucky, or 1 Ready.
- Flavor: "Shove the whole stack in. No looking back."
- NOTE: if the player has no gold, does nothing.

### Golden Pickaxe
- Tags: Prospector
- Rarity: Uncommon
- Effect: All gold gain is increased by 10%.
- Flavor: "This makes no sense."

## Special Artifacts

### Lethargic
- Tags: Corrupt
- Rarity: Corrupt
- Effect: The first match each combat generates no resources.
- Flavor: "Some mornings, you just aren't ready."

### Dry Atmosphere
- Tags: Corrupt
- Rarity: Corrupt
- Effect: All healing is reduced by 20%.
- Flavor: "Sun bakes the blood before it can heal."

### Tinnitus
- Tags: Corrupt
- Rarity: Corrupt
- Effect: Enemy intents are hidden on the first turn of each combat.
- Flavor: "The first shot always rings twice."

### Greed
- Tags: Corrupt
- Rarity: Corrupt
- Effect: All gold gain is reduced by 10%.
- Flavor: "Greed never leaves you richer, only hungrier."

## Misc Artifacts

### Merchant's Token
- Tags: -
- Rarity: Rare
- Effect: The next merchant's prices are 25% cheaper.
- Flavor: "Saturday is bargain day at the supermarket! I can't miss it!"
