...
const cardGame: EventDefinition = {
  id: 'card_game',
  title: 'The Card Game',
  flavourText: "A lantern sputters against the bruise-colored dusk, casting its glow across three warped cards and the ringed hand of a {{yellow:wiggle:raccoon}} whose own wanted poster peels on the wall behind him. The {{blue:breathe:ghost town}} beyond is hollow as a gunshot's echo — empty windows, leaning beams, not so much as a dog barking — and the only sound is the soft tick of his pipe and the slow, patient shuffle of pasteboard on pine.\nHe does not look up when you approach; he has been waiting, and the cards, it seems, {{red:jump:already know your name}}.",
  align: 'left',
...
const oldWell: EventDefinition = {
  id: 'old_well',
  title: 'The Old Well',
  flavourText: "A crescent moon presides over a stone well that breathes {{green:breathe:soft green light}} into the dusk, as though something below were dreaming in color. Coins spill from the dirt around its base and a weathered hat lies forgotten in the grass — tokens of travelers who leaned too far, or listened too long.\nThe bucket turns on its rope without a hand to crank it, and from the {{red:jump:dark throat}} of the shaft comes a faint, {{yellow:wiggle:patient jingling}}, like someone counting their winnings in the deep.",
  align: 'left',
...
const coyoteDen: EventDefinition = {
  ...
  flavourText: "The crescent moon hangs pale above the mesas as dusk bleeds purple over the badlands, and a cave yawns from the rock like a wound that never healed. Paw prints trail past a sun-bleached skull toward the dark within, where too many {{yellow:breathe:yellow eyes}} glint in unnatural stillness, unblinking, patient. {{yellow:shimmer:Something gold}} catches the last of the light inside that den, glimmering atop the bones of those who reached for it before.",
  align: 'left',
...
const trainWreck: EventDefinition = {
  ...
  flavourText: "Black smoke still curls from the {{silver:jump:iron belly}} of a locomotive run aground in the gulch, its cowcatcher buried in red dirt and its wheels locked mid-turn, as if the engine itself had seen something on the tracks worth dying over. Splintered crates bleed {{yellow:shimmer:gold}} and {{dark_grey:wiggle:gunpowder}} across the stones, a fallen hat and a lone rifle marking where men stood before the quiet took them, and the telegraph poles lean overhead like mourners at a grave.\nNo crows call, no wind stirs the broken bridge above, yet somewhere beneath the wreckage a slow, steady ticking keeps time with a heart that {{red:jump:should not still be beating}}.",
  align: 'right',
...
const abandonedMine: EventDefinition = {
  ...
  flavourText: "A crooked sign nailed to rotting timbers warns {{red:jump:KEEP OUT}} in letters the sun has nearly scoured away, but the rails still run straight into the mountain's red throat, past a pickaxe left mid-swing and a busted lantern leaking its last oil into the dust.\nDeep inside the shaft, {{teal:breathe:a teal light}} pulses where no lantern should burn, throwing long shadows across skulls and bone that nothing has bothered to bury. The vultures ride the thermals above the mesas, patient as stone, as though {{red:breathe:they already know}} which direction you will choose.",
  align: 'right',
...
const vultureCircle: EventDefinition = {
  ...
  flavourText: "The wind has stopped, like the desert itself is holding its breath. A bounty hunter lies face-down in cracked earth, his {{yellow:shimmer:badge still gleaming}} beneath a dark bloom of blood, the wanted posters on the wire fence fluttering with names he will never collect. The vultures have already chosen their judge, and one {{red:jump:watches from a dead branch}} with the patient eyes of something that has seen this story end a hundred times before.",
  align: 'left',
  ...
  choices: [
    ...
    {
      label: 'Bury him',
      description: 'Heal 11 HP and gain 33 gold from his pockets.',
      effect: 'vulture_bury',
...
const campfireStranger: EventDefinition = {
  ...
  flavourText: "A low fire {{orange:jump:crackles}} in a ring of stones beneath a sky so heavy with stars it feels like the heavens are {{yellow:wiggle:leaning in to listen}}. A stranger sits on a half-rotted log across the flames, hat pulled down, face lost to shadow, a tin coffee pot steaming quietly beside his boots as though he had set it out for a guest he already knew was coming.\nThe desert behind him is {{dark_purple:shimmer:black and endless}}, and though he has not spoken, the empty log on your side of the fire seems to have been waiting a very long time.",
  align: 'left',
...
const riggedBridge: EventDefinition = {
  ...
  flavourText: "A rope bridge slumps between two red canyon walls like a broken jaw, its planks warped and half-missing, its cables fraying in the dry wind above a drop deep enough to swallow a man's last words. A bundle of {{red:wiggle:dynamite}} has been strapped to the anchor post, fuse trailing down the rock like a black snake curled against the stone, and a second bundle waits on the cliff's edge as if someone left the choice behind on purpose.\nFar below, a thin river {{teal:shimmer:glints}} through the haze, patient as a grave digger, and the only sound is the {{red:breathe:slow groan}} of the ropes deciding whether to hold a little longer or not.",
  align: 'left',
...
const snakeCharmer: EventDefinition = {
  ...
  flavourText: "A hooded figure sits cross-legged on a threadbare rug beneath a bruised twilight sky, her lantern {{orange:breathe:burning low}} beside a cow's bleached skull and a scatter of bones, feathers, and beaded charms laid out like a dealer's cards. A rattlesnake rises from a painted clay pot at the sound of her wooden flute, {{green:wiggle:hood flared}}, {{silver:shimmer:fangs bared}}, while more of its kin coil quietly through the dust beyond the edges of the rug.\nShe watches you with a {{red:jump:knowing smile}}, eyes half hidden in shadow, as though she has already seen how long you will live after the bite.",
  align: 'left',
...
const ghostTownSaloon: EventDefinition = {
  ...
  flavourText: "The swinging doors hang {{red:wiggle:crooked}} on {{brown:wiggle:rusted}} hinges and cobwebs drape the bottles behind the bar like funeral lace, each label faded but the whiskey inside still bright as a fresh wound. A half-played hand of cards waits on a dusty table beside a lone revolver and a stack of chips, as if the game had paused mid-bet and the players simply forgot how to breathe.\nSunlight cuts through the torn curtains in {{yellow:shimmer:slow gold bars}}, and somewhere in the back room the piano ghosts a single key, though no hand rests upon it.",
  align: 'left',
  background: 'event_saloon.png',
  choices: [
    {
      label: 'Drink',
      description: 'Heal 18 HP. Lose 1 swap at the start of the next fight.',
      effect: 'saloon_drink',
  ...
  choices: [
    {
      ...
      description: 'Gain 1 Strong Whiskey and lose 15 gold.',
    ...
    {
      label: 'Buy Panacea',
      description: 'Gain 1 Panacea and lose 30 gold.',
      effect: 'medicine_panacea',
      resultText: 'The glass is cool in your palm, and whatever moves inside it goes still when Doc takes your coin.',
    },
    {
      label: 'Threaten him',
