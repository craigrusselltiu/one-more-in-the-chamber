...
export class CombatManager {
  ...
  getTargetedEnemy(): Enemy | null {
    ...
    return this.enemies.filter((e) => !e.state.isDead)[state.targetedEnemyIndex] ?? null;
  }
  ...
  activateDeadeye(): boolean {
    ...
    this.emitFullState();
    EventBus.emit(GameEvent.ABILITY_CHARGE_CHANGE, this.player.abilityCharge);
    return true;
  }
  ...
  deadeyeShoot(row: number, col: number): void {
    ...
    if (tile.isShowdown) {
      ...
      // The board will handle the actual clearing -- for now we process the tile
  ...
  private executeEnemyTurn(): void {
    ...
    for (const enemy of this.aliveEnemies()) {
      ...
      if (venomDamage > 0) {
        EventBus.emit(GameEvent.ENEMY_HP_CHANGE, enemy.state.id, enemy.state.health);
      }
    }
