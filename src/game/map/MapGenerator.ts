...
function assignNodeTypes(
  ...
  const targets: { type: MapNodeType; count: number }[] = [
    // L4 (wanted level): +eliteCountBonus to both min and max.
    { type: 'elite', count: pick(3 + eliteCountBonus, 4 + eliteCountBonus) },
    { type: 'merchant', count: pick(2, 3) },
    // L29 (wanted level): campfirePenalty shrinks the assignable campfire count.
    { type: 'campfire', count: Math.max(0, pick(2, 3) - campfirePenalty) },
...
export function generateMap(seed: string, act: Act, eliteCountBonus = 0, campfirePenalty = 0): MapState {
  ...
  assignNodeTypes(nodes, totalRows, rng, eliteCountBonus, campfirePenalty);
